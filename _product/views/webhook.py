...
from _product.services.payment_transition import SavingPaymentTransitionError, apply_saving_payment_update
from _setting.models import XenditSetting


@csrf_exempt
@require_POST
def saving_xendit_webhook(request):
    ...
    try:
        saving_txn = SavingTransaction.objects.get(reference_id=reference_id)
    except SavingTransaction.DoesNotExist:
        return JsonResponse({"detail": "Saving transaction not found."}, status=404)
    webhook_id = request.headers.get("webhook-id") or hashlib.sha256(request.body).hexdigest()
    if saving_txn.xendit_webhook_id == webhook_id:
        return JsonResponse({"status": "duplicate", "message": "duplicate"})
    try:
        apply_saving_payment_update(
            saving_txn, payload, webhook_id=webhook_id, webhook_payload=payload
        )
    except SavingPaymentTransitionError as exc:
